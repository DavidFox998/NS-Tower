...
HONESTY NOTES (read before citing this file):
  ...
  * The two genuine NS surfaces:
      - `enstrophy_bound_global_Surface`  (global H^1/enstrophy bound; discharging
        it is equivalent to 3D NS global regularity, i.e. Surface #1),
      - `leray_proj_ker_eq_grad_Surface`  (the global L^2-orthogonal Helmholtz
        decomposition `ker P = gradSubmodule`).
  * These are NS-only. They are independent of the YM mass gap and of the
    Kotecky-Preiss criterion; this registry does NOT bundle YM with NS.
...
bound -- equivalent to 3D NS global regularity -- and the global L^2-orthogonal
